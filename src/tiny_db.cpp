...
void initialSystem(){
	...
	bplus_tree tiny_db(dbFileName, (!is_file_exist(dbFileName)));
	tiny_db_ptr = &tiny_db;
	...
	while(true){
		...
		if(strcmp(userCommand,".exit") == 0){
			...
			if(okNum < 3){
				...
				int return_code = insertRecord(tiny_db_ptr,keyIndex,insertData);
			...
			if(okNum < 1){
				...
				int return_code = deleteRecord(tiny_db_ptr,keyIndex);
	    	...
	    	if( ! strstr (userCommand,"=")){
				...
				if(okNum < 2){
					...
					searchAll(tiny_db_ptr,&i_start, &i_end);
				...
				if(okNum < 1){
					...
					int return_code = searchRecord(tiny_db_ptr,keyIndex,return_val);
			...
			if(okNum < 3){
				...
				int return_code = updateRecord(tiny_db_ptr,keyIndex,updateData);
